/**
 * Provider 10 Adapter — Eris ✦
 * Endpoint Clowatch v4, skip pricing rule (harga raw dari provider langsung).
 *
 *   - Endpoint: https://api.clowatch.com/api/v4 (default)
 *   - API key terpisah (PROVIDER10_API_KEY), boleh fallback ke Earth's key
 ...
 * Sama treatment dengan Saturn (api3), Venus (api6), dan Uranus (api9) di FINAL_PRICE_PROVIDERS.
 */
...
async function fetchProvider(path: string, options: FetchOptions = {}): Promise<unknown> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Provider10Error(msg, res.status, obj.code, obj.retryAfterSec);
    }
...
export async function checkSms(orderId: number) {
  ...
  try {
    ...
    if (err instanceof Provider10Error && err.status === 404) {
