/**
 * Provider 8 Adapter — Mercury ☿️
 * Sama seperti Earth (api5), tapi:
 *   - Endpoint: https://api.clowatch.com/api/v2 (default)
 *   - API key terpisah (PROVIDER8_API_KEY), boleh fallback ke Earth's key
 *   - Mirror kontrak: /countries, /services?country=, POST /order, GET /order/:id, POST /order/:id/cancel
 *
 * Flat markup: +115 IDR diterapkan di atas pricing rule (ditambahkan via
 * lib/pricing.ts → applyServerExtraMarkup, BUKAN di adapter), supaya rule admin
 * Earth (api5) ikut diadopsi Mercury by default.
 */

const BASE_URL =
  process.env.PROVIDER8_API_URL || "https://api.clowatch.com/api/v2";
const API_KEY =
  process.env.PROVIDER8_API_KEY || process.env.PROVIDER5_API_KEY || "";
...
async function fetchProvider(path: string, options: FetchOptions = {}): Promise<unknown> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Provider8Error(msg, res.status, obj.code, obj.retryAfterSec);
    }
...
export async function checkSms(orderId: number) {
  ...
  try {
    ...
    if (err instanceof Provider8Error && err.status === 404) {
