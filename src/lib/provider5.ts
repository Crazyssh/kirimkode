...
async function fetchProvider(path: string, options: FetchOptions = {}): Promise<unknown> {
  ...
  // Order: pakai timeout panjang tapi TERBATAS (default via timeoutMs), JANGAN
  // nunggu selamanya — request gantung saat worker recycle = saldo hilang.
  // Call lain: 30 detik. noTimeout (legacy) = tanpa batas (dihindari untuk order).
...
export async function createOrder(negara: number, layanan: string, operator: string) {
  ...
  const raw = (await fetchProvider("/order", {
    ...
    body,
    skipCache: true,
    timeoutMs: 90000, // maks 90 detik — jangan nunggu selamanya (cegah saldo hilang saat worker recycle)
